...
<table id="datatable"  class="table table-striped table-bordered">
	...
	<tbody>
	<?php
	$sql ="SELECT * FROM billing LEFT JOIN guest ON billing.guestid=guest.guestid WHERE billing.status='Pending' AND billing.bill_type='Guest Payment' ORDER BY billing.billid DESC";
	...
	while($rs = mysqli_fetch_array($qsql))
	{
		...
		echo "<tr>	
			...
			<td>$rs[8]</td>	
			<td>";
		echo "<a href='viewguestpendingbookings.php?bookingid=$rs[0]&st=Approved' class='btn btn-info' style='width: 100%'>Approve</a>";
		echo "<hr>";
		echo "<a href='viewguestpendingbookings.php?bookingid=$rs[0]&st=Rejected'  class='btn btn-danger' onclick='return confirmdel()' style='width: 100%'>Reject</a>";
		echo "</td></tr>";
	}
	?>
	</tbody>
